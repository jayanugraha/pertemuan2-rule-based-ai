...
#Define Nilai Fungsi Keanggotaan Kolestrol
...
#Define Nilai Fungsi Keanggotaan Blood Sugar
blood_sugar_very_high = mf.trimf(x_blood_sugar, [90, 120, 130])

#Define Nilai Fungsi Keanggotaan LDL
...
rules ={
    'not_risk': sugeno_f(np.fmax(np.fmax(np.fmax(np.fmax(np.fmax(rule1,rule5),rule6),rule7),rule8),rule12), 'not_risk'),
    'little_risk': sugeno_f(np.fmax(np.fmax(rule2,rule13),rule16), 'little_risk'),
    'middle_risk': sugeno_f(np.fmax(np.fmax(rule3, rule9),rule22), 'middle_risk'),
    'high_risk': sugeno_f(np.fmax(np.fmax(np.fmax(rule4, rule10),rule14),rule17), 'high_risk'),
    'very_high_risk': sugeno_f(np.fmax(np.fmax(np.fmax(np.fmax(np.fmax(np.fmax(np.fmax(rule11,rule15),rule18),rule19),rule20),rule21),rule23),rule24), 'very_high_risk'),}


# DEFUZZIFIKASI
out_risk = np.fmax(np.fmax(np.fmax(np.fmax(rules['not_risk'], rules['little_risk']), rules['middle_risk']), rules['high_risk']), rules['very_high_risk'])
...
# OUTPUT
if defuzzified >= 0 and defuzzified <= 10 and np.max(rules['not_risk']) > np.max(rules['little_risk']):
    print("Diagnosed as Not Risk")
elif defuzzified >= 5 and defuzzified <= 20 and np.max(rules['not_risk']) < np.max(rules['little_risk']) and np.max(rules['little_risk']) > np.max(rules['middle_risk']):
    print("Diagnosed as Little Risk")
elif defuzzified >= 15 and defuzzified <= 30 and np.max(rules['little_risk']) < np.max(rules['middle_risk']) and np.max(rules['middle_risk']) > np.max(rules['high_risk']):
    print("Diagnosed as Middle Risk")
elif defuzzified >= 25 and defuzzified <= 40 and np.max(rules['middle_risk']) < np.max(rules['high_risk']) and np.max(rules['high_risk']) > np.max(rules['very_high_risk']):
    print("Diagnosed as High Risk")
elif defuzzified >= 35 and defuzzified <= 50 and np.max(rules['high_risk']) < np.max(rules['very_high_risk']):
